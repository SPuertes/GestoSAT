            ...
            <div class="col-sm-12">
                <section class="panel">
                    ...
                    <div class="panel-body">
                        ...
                        <% Map emps = ((GestoSAT)ses.getAttribute("gestor")).getEmpleados(); 
                            Iterator itEmpleado = emps.entrySet().iterator();
                            while(itEmpleado.hasNext()){
                               Map.Entry aux = (Map.Entry)itEmpleado.next(); 
                                if(!ses.getAttribute("id").toString().equals(aux.getKey().toString())){%>
                               <div class="conf-attr" <% out.print("id='"+aux.getKey()+"'"); %>>
                                <span class="empleado <% if(!((Empleado)aux.getValue()).isActivo()) out.print("borrado"); %>">
                                    <% out.print(((Empleado)aux.getValue()).getNombre()+" "+((Empleado)aux.getValue()).getApellidos()); %> 
                                </span>
                                    <a <% out.print("href='editarEmpleado.jsp?id="+aux.getKey()+"'"); %>><i class="fa fa-pencil"></i></a>
                                    <i class='fa fa-times sep'></i>
                                    <% if(!((Empleado)aux.getValue()).isActivo()) out.print("<i class='fa fa-eject sep'></i>");%>                              
                            </div>
                    <% }
                            } %>
                    </div>
